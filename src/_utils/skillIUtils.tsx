...
export const getTotalSkillValue = (
    ...
    return (skill.ranks || 0) + attributeMod + armorCheckPenalty + miscMods;
};
...
export const getModsForSkill = (skill: RankedSkill, character: Character) => {
    const skillsAffectedBySize = [SkillTypes.Stealth];
    const skillMods = character.miscModifiers.filter(
        (x) => x.skill === skill.name || !!x.allSkills
    );
    const sizeBonus = getSkillSizeBonus(character);
    const equipmentBonuses = getModifiersFromWornEquipment(character).filter(
        (x) => x.skill === skill.name || x.allSkills
    );
    ...
    const statusEffectMods = [
        ...
        ...getFascinatedModifiers(character, skill.name),
        ...getFearModifiers(character, skill.name),
        ...getSickenedModifiers(character, skill.name),
        ...getEnergyDrainedModifiers(character, skill.name),
    ].filter((x) => x.skill === skill.name);
...
export const canUseUntrained = (skill: RankedSkill) => {
