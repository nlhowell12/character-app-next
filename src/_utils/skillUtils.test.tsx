...
describe('Skill Utils', () => {
    ...
    test('getTotalSkillValue', () => {
        ...
        expect(
            getTotalSkillValue(
                { ...mock0, equipment: [...mock0.equipment, magicLeather] },
                mock0.skills.Acrobatics
            )
        ).toBe(5);
        expect(
            getTotalSkillValue(
                { ...mock0, equipment: [...mock0.equipment, magicLeather] },
                mock0.skills.Intimidate
            )
        ).toBe(0);
        expect(
            getTotalSkillValue(
                { ...mock0, equipment: [...mock0.equipment, magicLeather] },
                mock0.skills.Stealth
            )
        ).toBe(11);
    ...
    test('getArmorCheckPenalties', () => {
        ...
        expect(getArmorCheckPenalties([...mock0.equipment, magicLeather])).toBe(
            -2
        );
    });
    ...
    test('canUseUntrained', () => {
        ...
        expect(canUseUntrained(untrainedCanUse)).toBeTruthy();
        expect(canUseUntrained(untrainedCannotUse)).toBeFalsy();
        expect(canUseUntrained(trainedCanUse)).toBeTruthy();
    });
});
