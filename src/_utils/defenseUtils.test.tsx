...
describe('Defense Utilities', () => {
    ...
    test('getMiscAcBonuses', () => {
        const mock0Modifiers = mock0.miscModifiers?.filter(
            (mod) => !!mod.defense
        );
        expect(defenseUtils.getMiscAcBonuses(mock0)).toStrictEqual(
            mock0Modifiers
        );
    });
    ...
    test('getDefenseBonuses', () => {
        ...
        expect(
            defenseUtils.getDefenseBonuses({
                ...mock0,
                miscModifiers: [
                    ...mock0.miscModifiers,
                    {
                        id: '10',
                        value: 0,
                        attribute: AttributeNames.Wisdom,
            ...
            [BonusTypes.Armor]: 2,
            [BonusTypes.Untyped]: 2,
            [BonusTypes.Enhancement]: getTotalAttributeModifier(
                mock0,
    ...
    it('should get correct maximum dex mod from equipment', () => {
        ...
        expect(
            defenseUtils.getLowestEqDexMod([
                ...mock0.equipment,
                { ...mock0.equipment[0], maxDexBonus: 2 },
            ])
        ).toBe(2);
    });
    it('should get adjusted adjusted dex mod', () => {
        ...
        expect(
            defenseUtils.getAdjustedMaxDexMod({
                ...mock0,
                equipment: [
                    ...mock0.equipment,
                    { ...mock0.equipment[0], maxDexBonus: 2 },
    ...
    test('getTotalDefense', () => {
        ...
        expect(
            defenseUtils.getTotalDefense({
                ...mock0,
                statusEffects: [StatusEffects.Blinded],
            })
        ).toStrictEqual({
            dsBonus: 11,
            drBonus: 3,
        ...
        expect(
            defenseUtils.getTotalDefense({
                ...mock0,
                statusEffects: [StatusEffects.Frightened],
            })
        ).toStrictEqual({
            dsBonus: 13,
            drBonus: 3,
    ...
    test('getResistances', () => {
        const equipmentMod = {
            ...
            resistance: true,
            damageType: EnergyType.Fire,
        };
        const moddedCloak = {
            ...cloakOfResistance,
            modifiers: [equipmentMod],
        } as Equipment;
        const immunityMod = {
            id: '1',
            immunity: true,
    ...
    test('getTotalSaveBonus', () => {
        let mock = mock0;
        expect(
            defenseUtils.getTotalSaveBonus(mock, AttributeNames.Dexterity)
        ).toBe(7);
        ...
        expect(
            defenseUtils.getTotalSaveBonus(mock, AttributeNames.Strength)
        ).toBe(3);
        expect(
            defenseUtils.getTotalSaveBonus(mock, AttributeNames.Intelligence)
        ).toBe(11);
    });
    test('should not stack save bonuses of the same type', () => {
        let mock: Character = mock0;
        expect(
            defenseUtils.getTotalSaveBonus(mock, AttributeNames.Strength)
        ).toBe(-2);
        const mock2: Character = {
            ...mock,
            miscModifiers: [
                {
                    id: '1',
                    type: BonusTypes.Spiritual,
                    value: 2,
                    source: ModifierSource.spell,
                    allSaves: true,
                },
        ...
        expect(
            defenseUtils.getTotalSaveBonus(mock2, AttributeNames.Strength)
        ).toBe(3);
    });
});
